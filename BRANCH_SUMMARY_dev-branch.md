...
```
System Boot (multi-user.target)
    ↓
┌─────────────────────────────────────────┐
│  hw-management.service                  │
│  (Main chassis management initialization)
└────────────┬────────────────────────────┘
             │
    ┌────────┴─────────────────────────────────────────────┐
    │                                                      │
    ▼                                                      ▼
┌──────────────────────────────────────┐  ┌──────────────────────────────────────┐
│ hw-management-peripheral-updater     │  │ hw-management-thermal-updater        │
│                                      │  │                                      │
│ Purpose: Critical device monitoring  │  │ Purpose: Temperature monitoring      │
│ Priority: Must always run            │  │ Priority: Optional (customer choice) │
│                                      │  │                                      │
│ Monitors:                            │  │ Monitors:                            │
│  • Fans (sync_fan)                   │  │  • ASIC temperatures                 │
│  • Leakage sensors                   │  │  • Optical module temperatures       │
│  • Power button events               │  │                                      │
│  • BMC sensors (Redfish)             │  │ Behavior:                            │
│  • ASIC chipup status                │  │  • Polls at configured intervals     │
│                                      │  │  • 3s for ASICs, 20s for modules     │
│ Behavior:                            │  │                                      │
│  • Change-based triggering           │  │ Dependencies:                        │
│  • Only executes on value changes    │  │  • Uses helper functions from        │
│  • Efficient resource usage          │  │    peripheral_updater                │
└──────────────────────────────────────┘  └──────────────────────────────────────┘
```
...
```
hw_management_platform_config.py (Data Layer)
    │
    │ Provides: PLATFORM_CONFIG (centralized hardware definitions)
    │
    ├──────────────────────────────┬────────────────────────────────┐
    │                              │                                │
    ▼                              ▼                                ▼
hw_management_lib.py      peripheral_updater.py          thermal_updater.py
(Logging)                 (Core Service)                 (Optional Service)
                                  │
                                  │ Exports:
                                  │  • get_asic_num()
                                  │  • update_asic_chipup_status()
                                  │
                                  └───────────────────────────────► (imported by thermal)
```
