...
characteristics (from `hw-management.sh` `sn66xxld_specific()` for SKU HI193,
`tests/system_tree/hw-management_val_sn6600.txt`, and
`tests/system_tree/hw-management_tree-SN6600_LD.txt`):

| Component | Value | Description |
|-----------|-------|-------------|
| ASICs | 1 | Single ASIC (`config/asic_num` = 1) |
| CPLDs | 4 | System control CPLDs (`config/cpld_num` = 4) |
| Leakage sensors (config) | 2 | `config/leakage_counter` = 2 |
...
| `config/i2c_comex_mon_bus_default` | COMEX I2C bus index for udev filtering | Written by `sn66xxld_specific()` as **6**; sample `hw-management_val_sn6600.txt` shows **5**; COMEX hwmon paths in tree use `i2c-5` |
...
The validated sysfs tree (`hw-management_tree-SN6600_LD.txt`) lists **19**
...
**Note:** `config/leakage_counter` is **2**; the tree also lists `leakage3`
through `leakage5` under `system/`.

---

## LEDs

Nodes under `led/` match `hw-management_tree-SN6600_LD.txt` (power / status /
...
- Config sample values:
  `tests/system_tree/hw-management_val_sn6600.txt`
- Sensors:
