...
| Fan modules | 8 | Hot-plug fan drawers (`hotplug_fans` = 8, `max_fans` = 8) |
| Fan tachometers | 16 | Two tachos per drawer (front + rear) in `sn66xxld_sensors.conf`; kernel fan CPLD exposes up to **20** tacho registers |
...
replaces liquid-cooled PDB / leakage infrastructure with **8 fan drawers** and
...
| `config/hotplug_fans` | Hot-plug fan drawer count | 8 |
...
| `config/fan_max_speed` | Global fan max RPM reference | 24000 |
| `config/fan_min_speed` | Global fan min RPM reference | 6468 |
| `config/fan_front_max_speed` | Front (inlet) fan max RPM | 24000 |
| `config/fan_front_min_speed` | Front (inlet) fan min RPM | 6600 |
| `config/fan_rear_max_speed` | Rear (outlet) fan max RPM | 21000 |
| `config/fan_rear_min_speed` | Rear (outlet) fan min RPM | 6600 |
| `config/psu_fan_max` | PSU internal fan max RPM | 27500 |
| `config/psu_fan_min` | PSU internal fan min RPM (20% of max) | 5500 |
| `config/tc_config.json` | Thermal control policy | Copied from `tc_config_msn6600.json` at init (see [Thermal Control](#thermal-control)) |

**Note:** `config/lm_sensors_config` is a symlink to
`/etc/hw-management-sensors/sn66xxld_sensors.conf`.

**Note:** `sn66xx_specific()` does **not** set `lm_sensors_labels` for HI186.
Label definitions for tooling exist in `n66x_sensors_labels.json`
(`labels_HI186_rev1_array`) but are not linked into `config/lm_sensors_labels`
by default platform init.
...
| `environment/psu<N>_in3_input` | DC output voltage (54 V rail) | mV |
...
**0x5a**, **0x5b**, **0x5c** (logical numbering; runtime `config/psuX_i2c_addr`
...
Eight hot-plug fan **drawers** (4 front + 4 rear redundant groups per platform
description). Each drawer carries two tachometer channels (inlet + outlet).

### Fan drawer hot-plug

Kernel mlxreg-io exposes drawer presence as `fan1`..`fan8` (see
`nvsw_host_spc6_hid186_regs_io_data[]`). These drive:
...
(`mlxreg_fan-isa-*` in sensors.conf, up to **16** labeled channels):
...
| … | … | … |
| `fan15`, `fan16` | Drawer 8 | Tach 1, Tach 2 |
...
**Kernel note:** `nvsw_host_cpld_hid186_fan_data[]` registers **20** tacho
channels (`tacho1`..`tacho20`); `config/max_tachos` is populated at runtime
...
| Node | Description | Values |
|------|-------------|--------|
| `events/fan1` .. `events/fan8` | Fan drawer hot-plug | 0=removed, 1=inserted |
| `events/psu1` .. `events/psu4` | PSU module hot-plug | 0=removed, 1=inserted |
| `events/pwr1` .. `events/pwr4` | AC input **cable** hot-plug | 0=unplugged, 1=plugged |

**Note:** `events/pwr<N>` reflects AC power **cable** presence
...
mlxreg-io `fan1`..`fan8`, `psu1`..`psu4`, `pwr1`..`pwr4`, and ASIC chip-up
status.
...
| Item | Source |
|------|--------|
| Init target | `thermal_control_config="$thermal_control_configs_path/tc_config_msn6600.json"` |
...
|-----------|-------|
| Airflow profile | `C2P` |
| Fan drawers in sensor list | `drwr1`..`drwr8` |
| PSUs in sensor list | `psu1`..`psu4` |
| Front fan RPM range | 6600 – 24000 |
| Rear fan RPM range | 6600 – 21000 |
| PWM floor | 30% |
...
- Sensors:
  `usr/etc/hw-management-sensors/sn66xxld_sensors.conf`
- Optional labels:
  `usr/etc/hw-management-sensors/n66x_sensors_labels.json`
  (`labels_HI186_rev1_array`)
- Thermal policy:
