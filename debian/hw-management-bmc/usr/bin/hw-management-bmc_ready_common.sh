...
{
    USER_NAME=$1
    user_groups=$(busctl get-property xyz.openbmc_project.User.Manager /xyz/openbmc_project/user/${USER_NAME} xyz.openbmc_project.User.Attributes UserGroups)
    if [[ "$user_groups" == *"ipmi"* ]]; then
        echo "Remove ipmi permissions from user ${USER_NAME}"
        busctl set-property xyz.openbmc_project.User.Manager /xyz/openbmc_project/user/${USER_NAME} xyz.openbmc_project.User.Attributes UserGroups as 3 "hostconsole" "ssh" "redfish"
        user_groups=$(busctl get-property xyz.openbmc_project.User.Manager /xyz/openbmc_project/user/${USER_NAME} xyz.openbmc_project.User.Attributes UserGroups)
        if [[ "$user_groups" == *"ipmi"* ]]; then
...
create_nosbmc_user()
{
   ...
   CHANNEL=1
   user_groups=$(busctl get-property xyz.openbmc_project.User.Manager /xyz/openbmc_project/user/${USER_NAME} xyz.openbmc_project.User.Attributes UserGroups 2>/dev/null)
   if [ $? -eq 0 ]; then
     ...
     busctl call xyz.openbmc_project.User.Manager /xyz/openbmc_project/user xyz.openbmc_project.User.Manager CreateUser sassb "$USER_NAME" 4 {"ipmi","redfish","ssh","hostconsole"} "priv-admin" true
	 sleep 2
