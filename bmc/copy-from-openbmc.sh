...
openbmc_script_dest() {
	...
	case "$base" in
		...
		a2d_leakage_read.sh) echo hw-management-bmc-a2d-leakage-read.sh ;;
		bios-recovery-flash.sh) echo hw-management-bmc-bios-recovery-flash.sh ;;
		max1363_force_alarm.sh) echo hw-management-bmc-max1363-force-alarm.sh ;;
		max1363_read_status.sh) echo hw-management-bmc-max1363-read-status.sh ;;
		*) add_prefix "$base" ;;
	esac
}
...
# BMC ready init: common for SONiC BMC (not under usr/etc/<HID>/)
cp "$A/spc6-ast2700-a1-bmc_ready.sh" "$BD/usr/usr/bin/hw-management-bmc-ready.sh"
chmod +x "$BD/usr/usr/bin/hw-management-bmc-ready.sh"
# Platform-specific scripts -> etc/HI189
