...
|-------------|------------------|---------|
| meta-nvidia/.../meta-**spc6-ast2700-a1**/.../files/71-hw-management-events.rules | **usr/etc/HI189/71-hw-management-bmc-events.rules** | **`hw-management-bmc-plat-specific-preps`** copies to **`/lib/udev/rules.d/`** at boot |
...
On the target system the **hw-management-bmc** Debian package installs this tree as **`/usr/etc/HI189/`** (not under `/etc/`). At boot, `hw-management-bmc-plat-specific-preps` copies from `/usr/etc/<HID>/` into `/etc/`, **`/etc/modprobe.d/`** ( **`hw-management-bmc.conf`** ), `/usr/bin/`, and `/lib/udev/rules.d/` as needed.
...
| .../spc6-ast2700-a1/.../spc6-ast2700-a1-bmc_ready.sh | usr/etc/HI189/ (platform-specific) |
...
# udev
cp "$OPENBMC/$S/$A/71-hw-management-events.rules" bmc/usr/etc/HI189/71-hw-management-bmc-events.rules
