...
|-------------|------------------|---------|
| meta-nvidia/.../meta-**spc6-ast2700-a1**/.../files/71-hw-management-events.rules | **usr/etc/HI193/71-hw-management-bmc-events.rules** | **`hw-management-bmc-plat-specific-preps`** copies to **`/lib/udev/rules.d/`** at boot |
...
| Source file | Destination |
|-------------|--------------|
| .../spc6-ast2700-a1/.../a2d_leakage_config.json | usr/etc/HI193/**hw-management-bmc-a2d-leakage-config.json** |
| .../spc6-ast2700-a1/.../platform_config | usr/etc/HI193/**hw-management-bmc-platform.conf** |
| .../spc6-ast2700-a1/.../spc6-bmc.conf | usr/etc/HI193/**hw-management-bmc.conf** (→ **`/etc/modprobe.d/hw-management-bmc.conf`** at boot) |
| *(maintain in repo per platform)* | usr/etc/HI193/**hw-management-bmc-network.conf** (→ **`/etc/hw-management-bmc-usb0.conf`**, drives **`usb0`** **`Address=`** in generated **`.network`**) |
...
| .../spc6-ast2700-a1/.../spc6-ast2700-a1-hw-management-events.sh | usr/etc/HI193/**hw-management-bmc-events.sh** |
...
# udev
cp "$OPENBMC/$S/$A/71-hw-management-events.rules" bmc/usr/etc/HI193/71-hw-management-bmc-events.rules

# Platform HI193
cp "$OPENBMC/$S/$A/a2d_leakage_config.json" bmc/usr/etc/HI193/hw-management-bmc-a2d-leakage-config.json
cp "$OPENBMC/$S/$A/platform_config" bmc/usr/etc/HI193/hw-management-bmc-platform.conf
