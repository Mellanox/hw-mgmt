...
| *(SONiC BMC reset-cause exporter)* | usr/usr/bin/**hw-management-bmc-get-reset-cause.sh** — primary **`reset_pwr_cycle`** / **`reset_soft_reboot`** / **`reset_unknown`** under **`/var/run/hw-management/bmc/`** (v3 SRST/EXTRST+WDT+ABR; W1C-clear **exported snapshot bits only**); domains incl. **`reset_abr`**; raw **`raw_scu*`** at bmc root. |
