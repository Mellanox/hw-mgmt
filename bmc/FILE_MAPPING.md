...
MCTP-over-IRoT udev rules ship as **`usr/etc/<HID>/99-hw-management-bmc-mctp.rules`**; **`hw-management-bmc-plat-specific-preps`** symlinks **`*.rules`** from **`/etc/<HID>/`** to **`/lib/udev/rules.d/`** on boot (same as other platform udev rules).
...
| *(SONiC BMC reset-cause exporter)* | usr/usr/bin/**hw-management-bmc-get-reset-cause.sh** — primary **`reset_pwr_cycle`** / **`reset_soft_reboot`** / **`reset_unknown`** under **`/var/run/hw-management/bmc/`** (v2, atomic temp+mv per boot); hardware **`reset_*`** under **`…/bmc/domains/`**; removes pre-v2 root **`reset_power_on`**, … after publish; raw **`raw_scu*`** at bmc root. |
