...
| [hw-management-bmc-system-sysfs.txt](hw-management-bmc-system-sysfs.txt) | **`/var/run/hw-management/system/`**: mlxreg-io (**regio**) + mlxreg-hotplug labels from **`nvsw_bmc_hid189_*`** (kernel patch), udev **`71-hw-management-bmc-events.rules`**, GPIO symlinks from **`hw-management-bmc-gpio-pins.json`**. |
