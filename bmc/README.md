...
**Per-`Device` JSON (optional):** **`Probe`** — when **`true`**, the kernel driver is bound via **`/sys/bus/i2c/devices/i2c-<bus>/new_device`** *before* **`CfgReg`** / threshold **`i2ctransfer`** writes so the driver does not overwrite programmed values afterward. **`Scale`**, **`CriticalMax`**, **`EmergencyMax`**, **`LoThreshRegVal`**, **`HiThreshRegVal`** feed the channel files above. Driver names: **`MAX1363`** → `max1363`, **`ADS7142`** → `ads7142` (adjust for your kernel). **`input`** symlinks are created only after configuration; if sysfs names differ on your board, extend **`find_iio_channel_raw`** in the script.
