...
| Topic | Policy |
|-------|--------|
| **Source priority** | For each SCU log: **`fw_printenv -n`** on **`reset_cause_scu0_0`**, **`reset_cause_scu0_2`**, **`reset_cause_scu1_0`**, **`reset_cause_scu1_3`**; if missing, same names from **`/proc/cmdline`** (U-Boot **`snap_bootargs`**); else **`devmem`** at **`0x12c02050`**, **`0x12c02070`**, **`0x14c02050`**, **`0x14c02080`**. |
...
### SONiC U-Boot snap bootenv (`hw-management-bmc-ready.sh`)

On **SONiC BMC OS** only (**`/etc/sonic/sonic_version.yml`** present), **`bmc_ready_sequence`** may patch eMMC U-Boot env so **`sonic_image_1`** / **`sonic_image_2`** run **`snap_sonic_postboot`** after **`sonic_bootargs`** / **`sonic_bootargs_old`**. That restores the reset-cause snap chain after the SONiC installer rewrites **`sonic_image_*`**.

| Topic | Policy |
|-------|--------|
| **When it runs** | SONiC rootfs only; no-op on OpenBMC images (no **`sonic_version.yml`**). |
| **U-Boot prerequisite** | **`fw_printenv -n`** must return non-empty **`snap_sonic_postboot`**, **`snap_reset_cause`**, and **`snap_bootargs`** (OpenBMC U-Boot 0004 default env). If any macro is missing, skip patching (info log); SONiC keeps stock **`sonic_image_*`**. |
| **`fw_setenv` unavailable** | Skip (info log); return success. |
| **BMC_READY** | Ensure and **`hw-management-bmc-get-reset-cause.sh`** failures are logged as warnings only; **`bmc_ready_sequence`** does not exit on them. |
| **Without U-Boot 0004** | SONiC boot unchanged; **`/proc/cmdline`** has no **`reset_cause_*`** tokens; **`get-reset-cause`** may still succeed via **`devmem`**. OpenBMC RAM **`bootcmd`** snap path is U-Boot scope, not this package. |
...
| **hw-management-bmc-init** | oneshot | `/bin/bash /usr/bin/hw-management-bmc-ready.sh` | `WantedBy=multi-user.target` | **`After=`** `local-fs`, **`hw-management-bmc-early-i2c-init`**. **`Requires=`** early-i2c-init. **`Before=`** `hw-management-bmc-boot-complete`. **`ConditionPathExists=`** `/usr/bin/hw-management-bmc-ready.sh`. On SONiC BMC, ready may patch **`sonic_image_*`** for **`snap_sonic_postboot`** when U-Boot snap macros exist (see **SONiC U-Boot snap bootenv**). |
