...
| **`hw-management-bmc-generate-dump.sh`** | SONiC BMC debug bundle (same idea as host **`hw-management-generate-dump.sh`**): **`dmesg`**, **`proc/interrupts.txt`**, **`network/ifconfig.txt`** (**`ifconfig -a`**, else **`ip addr show`**), **`i2c/`** (**`i2cdetect -l`**, **`i2cdetect-l_grep-v-mux.txt`**, per-bus **`i2cdetect -y <N>`** for lines from **`i2cdetect -l \| grep -v mux`**), CPLD via **`take_cpld_dump`**, **`systemctl`** for **`hw-management-bmc*`**, **`/var/run/hw-management`** tree + values (EEPROM: **`hexdump -C`**). Default **`/tmp/hw-mgmt-bmc-dump.tar.gz`**. Requires **bash**. |
...
| **`hw-management-bmc-generate-dump.sh`** | **bash** | **`#!/bin/bash`**: **`[[`**, **`source`** **`hw-management-bmc-cpld-dump.sh`** (**`take_cpld_dump`**), **`find`**, **`timeout`**. Uses **`tar cf - \| gzip -9`** (not GNU **`tar -I`**) and **`ls -ld`** per path (not GNU **`find -ls`**) so BusyBox **tar/find** work; **`readlink_canonical`** if **`readlink -f`** missing. Needs **`gzip`** in **`PATH`** (BusyBox or GNU). |
...
| **hw-management-bmc-plat-specific-preps** | oneshot | `/usr/bin/hw-management-bmc-plat-specific-preps.sh` | `WantedBy=sysinit.target` | **`After=local-fs.target`**. **`Before=`** `hw-management-bmc-early-config`, **`systemd-modules-load`**, **`basic.target`**, **`systemd-udevd`**, **`systemd-networkd`**. Deploys **`/etc/<HID>/`** (fallback **`/usr/etc/<HID>/`** on older images): **symlinks** **`*.sh`** → **`/usr/bin/`**, **`*.rules`** → **`/lib/udev/rules.d/`** (except **`99-hw-management-bmc-mctp.rules`**), **`hw-management-bmc.conf`** → **`/etc/modprobe.d/hw-management-bmc.conf`**; **copies** **`*.json`**, **`hw-management-bmc-platform.conf`**, eeprom/boot-complete conf into **`/etc/`** (see **Platform deploy**). Also copies or generates **`hw-management-bmc-network.conf`** / **`/etc/hw-management-bmc-usb0.conf`** and renders **`/etc/systemd/network/00-hw-management-bmc-usb0.network`** (see **USB0 / systemd-networkd** below). |
...
- **`/lib/udev/rules.d/`** — **symlinks** for each **`*.rules`** except **`99-hw-management-bmc-mctp.rules`** (e.g. **`5-hw-management-bmc-events.rules`**)
