...
| **CPLD aux fixup (boot-complete)** | After **`hw-management-bmc-boot-complete.sh`** sysfs thresholds are met, if **`/var/run/hw-management/system/reset_aux_pwr_or_fu`** or **`reset_aux_pwr_or_ref`** reads **1**, set **`reset_pwr_cycle=1`** and clear **`reset_soft_reboot`** / **`reset_unknown`**. Covers **aux_pwr_cycle then remote reboot** where SCU primary flags are ambiguous; plain remote reboot (CPLD **0**) is unchanged. Does not modify **`bmc/domains/`** or **`raw_scu*`**. |
| **Operator tools** | **`hw-management-bmc-get-reset-cause.sh`** re-reads SCU and re-exports files. **`hw-management-bmc-show-reset-cause.sh bmc`** shows primary; **`bmc-domain`** / **`bmc-raw`** show detail. After boot, init runs **`get-reset-cause`** once; boot-complete may adjust primary flags when CPLD aux is set. |
...
| **`hw-management-bmc-boot-complete.sh`** | Yes | **`#!/bin/sh`**: waits on **`/var/run/hw-management/`** entry counts vs **`/etc/hw-management-bmc-boot-complete.conf`**. On success, may apply CPLD **`reset_aux_pwr_or_*`** fixup to primary **`reset_pwr_cycle`**. |
