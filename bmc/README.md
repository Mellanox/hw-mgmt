...
| **Reset-log ownership** | U-Boot must preserve the AST2700 logs until Linux; the OpenBMC **`0005-ast2700-keep-reset-event-logs-for-post-mortem-inspec.patch`** is selected for images with the **`bmc-os-loader`** distro feature. The SONiC-only kernel **`0014-watchdog-aspeed-Preserve-AST2700-reset-event-logs.patch`** keeps `aspeed_wdt_probe()` from clearing SCU0 **0x070**. This exporter then owns the snapshot-and-clear handoff. The two kernel patch tables do not deploy the U-Boot patch. |
