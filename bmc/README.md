...
| `/var/run/hw-management/leakage/N/<j>/` | Per-channel directory. **`j`** is the mapped **hardware A2D input** from **`Channels[].Id`**, so it can be **non‑contiguous** (e.g. **`Channels: [{Id:1},{Id:4}]`** → dirs **`1/`** and **`4/`**). Legacy: `1` … **`NumChnl`** for a scalar/absent **`ChannelId`**, or the 1‑based **`Device[]`** index in per‑channel device‑map mode — see *Channel selection modes* below |
| `/var/run/hw-management/leakage/N/<j>/input` | If **`Probe`** is true: symlink to the kernel IIO raw attribute for the **wired hardware channel** — **`in_voltage<Id-1>_raw`** (legacy **`in_voltage<ChannelId-1>_raw`**, or **`in_voltage<j-1>_raw`** when absent). **Unscaled** sample |
| `/var/run/hw-management/leakage/N/<j>/min` | Low threshold from **`LoThreshRegVal`** (bytes → unsigned, big-endian) × **`Scale`**; fallback to the channel type's **`Types[].NormalMin`**, then **`Types[].WarningMin`** (legacy: device‑level) |
| `/var/run/hw-management/leakage/N/<j>/max` | High threshold from **`HiThreshRegVal`** × **`Scale`**; fallback to the channel type's **`Types[].NormalMax`**, then **`Types[].WarningMax`** (legacy: device‑level) |
| `/var/run/hw-management/leakage/N/<j>/warn` | Channel type's **`Types[].WarningMax`** (legacy: device‑level **`WarningMax`**) |
| `/var/run/hw-management/leakage/N/<j>/crit` | Channel type's **`Types[].CriticalMax`** (legacy: device‑level **`CriticalMax`**) |
| `/var/run/hw-management/leakage/N/<j>/lwarn` | Channel type's **`Types[].WarningMin`** (optional low-side warning bound; legacy: device‑level **`WarningMin`**) |
| `/var/run/hw-management/leakage/N/<j>/lcrit` | Channel type's **`Types[].CriticalMin`** (optional low-side critical bound; legacy: device‑level **`CriticalMin`**) |
| `/var/run/hw-management/leakage/N/<j>/type` | **`Channels[].Type`** for this channel (sensor kind label, e.g. **`rop`**, **`flex`**, **`embedded`**; legacy: device‑level **`Type`**) |
| `/var/run/hw-management/leakage/N/<j>/scale` | **`Scale`** from JSON (device‑level; one ADC scale for all channels) |
| `/var/run/hw-management/leakage/N/<j>/channel_name` | **`ChnlNames`** entry in **logical** order (**`ChnlNames[k-1]`** for logical channel *k*; note **`j`** may differ from *k* when **`Channels[].Id`** maps to a non‑sequential input) |

**Per-`Device` JSON (optional):** By default (**BOM‑alternative mode**) **`Device`** is an ordered list of alternatives — the first entry whose presence probe and **`configure_device`** step succeed is used (remaining entries are skipped). Put the BOM you want to win first (e.g. **`MAX1363`** before **`ADS1015`** when both can bind at the same address). Optional **`HW_MANAGEMENT_BMC_A2D_USE_ADS_HEURISTIC=1`** restores legacy behavior: skip **`MAX1363`** when the bus looks like **ADS1015** by register read. **`Probe`** — when **`true`**, the kernel driver is bound via **`/sys/bus/i2c/devices/i2c-<bus>/new_device`** *before* **`CfgReg`** / threshold **`i2ctransfer`** writes so the driver does not overwrite programmed values afterward. Thresholds are **per channel type**: each **`Device`** carries a **`Types[]`** array (one entry per kind — **`rop`** / **`flex`** / **`embedded`** — with **`WarningMax`**, **`CriticalMax`**, **`WarningMin`**, **`CriticalMin`**, and for ADS1015/ADS7924 **`NormalMin`** / **`NormalMax`**) and a **`Channels[]`** array (one **`{Id, Type}`** per logical channel). A channel's runtime files come from the **`Types[]`** entry whose **`Type`** matches that channel's **`Channels[].Type`**, so two channels of different types on one device get different bounds. **`Scale`**, **`LoThreshRegVal`**, **`HiThreshRegVal`** stay at the device level (register/scale are chip‑wide). Threshold values from registers are scaled by **`Scale`**; user limits **`Warning*`** / **`Critical*`** are stored as given in configuration units. (Legacy device‑level **`Type`** + **`Warning*`** / **`Critical*`** / **`Normal*`** + **`ChannelId`** are still honored when no **`Types[]`** / **`Channels[]`** are present.) Driver names: **`MAX1363`** → `max1363`, **`ADS1015`** → `ads1015` (adjust for your kernel). **`input`** symlinks are created only after configuration; if sysfs names differ on your board, extend **`find_iio_channel_raw`** in the script.
...
- **BOM‑alternative (default):** entries are mutually‑exclusive populations (e.g. **`MAX1363`** *or* **`ADS1015`**, often at the same address). The first present/working entry wins and drives all **`NumChnl`** channels. Use a **`Channels[]`** array here to map each logical channel to its wired hardware input and type (see below).
- **Per‑channel device‑map:** triggered when the number of **`Device[]`** entries equals **`NumChnl`**, every **`Bus`**/**`Address`** pair is distinct, *and* no entry uses a **`Channels[]`** (or array **`ChannelId`**). Then each entry is a separate sensor for one logical channel — entry *k* → channel **`k`** — and each is configured and linked independently.

**Channel mapping (`Channels[]`):** the preferred schema is a **`Channels[]`** array of **`{Id, Type}`** objects, one per logical channel:

- **`Id`** (1‑based) is the **hardware A2D input** the channel is wired to (AINx = `Id-1`), allowing non‑sequential inputs (holes).
- **`Type`** selects the threshold set from the device **`Types[]`** array (**`rop`** / **`flex`** / **`embedded`**).

A multi‑entry **`Channels[]`** means a single present chip serves all **`NumChnl`** channels — it **forces BOM‑alternative mode**, and each runtime **channel directory is named by its mapped `Id`** (e.g. **`Channels: [{Id:1},{Id:4}]`** → dirs **`1/`** and **`4/`**). **`Channels[].Id`** controls, per channel:

- the **`input`** symlink target — **`in_voltage<Id-1>_raw`** (e.g. input **`4`** → **`in_voltage3_raw`**);
- the **ADS1015** MUX programmed for the mapped channel(s) — config high byte **`0xc2/0xd2/0xe2/0xf2`** for inputs 1–4 (the first **`CfgRegVal`** byte is a placeholder; the script programs each mapped input);
- the **MAX1363** scan/monitor bytes in **`CfgRegVal`** — scan‑to‑CS **`0x01/0x03/0x05/0x07`** and monitor tag **`0x11/0x13/0x15/0x1f`** for inputs 1–4, patched from the mapped inputs (first mapped input for the monitor tag).

**Legacy `ChannelId`:** still supported when **`Channels[]`** is absent — a **scalar** (e.g. **`"ChannelId": 4`**, per‑channel device‑map mode) or an **array** (e.g. **`"ChannelId": [1, 4]`**, forces BOM‑alternative mode). When both are absent **`Id`** defaults to the logical channel index, and the channel type comes from the device‑level **`Type`**.

Note: in **per‑channel device‑map** mode a missing **`Device[0]`** leaves a hole at channel **`1`** (the next present device lands on its own **`Device[]`** index). When one chip provides several channels, prefer a **`Channels[]`** array on a single BOM‑alternative entry so the channel directories carry the wired input numbers.
...
- **Host handshake**: the host package runs **`hw-management-bmc-nvme-ready.sh`** on **`cpu_shutdown_req`** (peripheral-updater / chassis-events). Peripheral-updater polls that node when **`hw_management_platform_config.py`** lists it (including HI180, HI181, HI182/HI187/HI188, HI185, HI193/HI183). It flushes NVMe namespaces and then asserts **`cpu_power_off_ready`**. Duplicate helpers wait at most **1 s** for **`flock`**. If **`graceful_power_off`** exists, the helper must observe **`1`** before flush; any later **`0`** is sticky fail-closed (BMC 1 s re-arm). **`sync`** / **`blockdev --flushbufs`** run in **1 s** slices under a flush window (default **3** seconds, **`HW_MGMT_BMC_NVME_FLUSH_TIMEOUT_SECS`**) and **`HW_MGMT_BMC_NVME_ACK_CAP_SECS`** (default **3**) from process start so 1 Hz detect (up to 1s) plus the cap stay under BMC **`REBOOT_TO`** (5s). After writing ready, the helper re-validates and retracts to **`0`** if re-arm raced the write. Flush or timeout failure does **not** assert ready; this BMC wait then expires and power proceeds. Generation is bumped only by the lock owner. Chassis-events does not start the helper when **`cpu_shutdown_req`** is already 1. Peripheral-updater runs the helper before chassis-events (**`HW_MGMT_NVME_READY=0`**).
