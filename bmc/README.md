...
3. **Multi-user:** sync U-Boot **`ethaddr`** from BMC FRU fallback → BMC init (ready script) → boot-complete (ready-common + sysfs entry-count gate), plus health monitor; optional I2C recovery stack if **`/etc/hw-management-bmc-recovery.conf`** exists.
