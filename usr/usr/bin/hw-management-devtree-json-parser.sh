...
#
# JSON Parser Library for hw-management device tree
#
# This library provides JSON parsing functions using only AWK and standard
# utilities (no jq or Python required).
#
# Ported from bmc/usr/usr/bin/hw-management-bmc-json-parser.sh for SONiC
# x86 hw-management device tree.
#
# Usage:
...
# <index> is 0-based (first element is 0). Matches callers: devtree.
...
json_get_nested_array_element()
{
    ...
    }
    {
        ...
        # End of array (only when not inside a nested object at depth 0)
        if (in_target_array && !in_element && index($0, "]") > 0 && depth == 0) {
            in_target_array = 0
            next
        }

        # Find array elements: only a "{" at depth 0 starts a new element (not nested "{")
...
# Note: Must find the value after the *requested* key.
json_get_string()
{
    local key="$1"
    awk -v k="$key" '
...
json_validate()
{
    ...
    open_brackets=$(grep -o '\[' "$json_file" | wc -l)
    close_brackets=$(grep -o '\]' "$json_file" | wc -l)
