#!/usr/bin/env python3
"""
Unit tests for thermal control initialization and signal handling fixes.
...
class TestThermalInitAndSignalHandling(unittest.TestCase):
    """Test class for thermal control initialization and signal handling fixes."""
    ...
    @patch('hw_management_thermal_control.ThermalManagement.load_configuration')
    @patch('hw_management_thermal_control.Logger')
    @patch('hw_management_thermal_control.ThermalManagement.check_file')
    @patch('hw_management_thermal_control.ThermalManagement.read_file')
    @patch('hw_management_thermal_control.ThermalManagement.is_pwm_exists')
    @patch('signal.signal')
    def test_early_termination_with_initialized_sys_config(self, mock_signal, mock_is_pwm_exists,
        ...
        """Test that early termination works correctly with initialized sys_config."""
        ...
        with patch('hw_management_thermal_control.str2bool', return_value=True):
            tc = thermal_control.ThermalManagement(self.cmd_arg)
    ...
    def test_configuration_loading_failure_with_exception_handling(self, mock_is_pwm_exists,
        ...
        """Test that configuration loading failures are handled gracefully with proper exception handling."""
        ...
        for test_exception in test_exceptions:
            with self.subTest(exception=type(test_exception).__name__):
                with patch('hw_management_thermal_control.ThermalManagement.load_configuration',
                           side_effect=test_exception):

                    # Verify that initialization fails gracefully with sys.exit(1)
                    with self.assertRaises(SystemExit) as cm:
                        thermal_control.ThermalManagement(self.cmd_arg)
    ...
    @patch('hw_management_thermal_control.ThermalManagement.check_file')
    @patch('signal.signal')
    def test_signal_handler_with_uninitialized_state_protection(self, mock_signal, mock_check_file, mock_logger_class):
        """Test that signal handler handles uninitialized sys_config gracefully."""

        # Setup mocks
        mock_logger_class.return_value = self.mock_logger
        mock_check_file.return_value = False

        # Create a ThermalManagement instance but intercept before load_configuration
        with patch('hw_management_thermal_control.ThermalManagement.load_configuration') as mock_load_config:
            # Make load_configuration block, simulating slow config loading
            load_config_called = Event()

            def slow_load_config():
                load_config_called.set()
                time.sleep(0.1)  # Simulate slow loading
                return self.valid_sys_config

            mock_load_config.side_effect = slow_load_config
            ...
            # Start ThermalManagement initialization in a way that we can interrupt it
            with patch('hw_management_thermal_control.ThermalManagement.is_pwm_exists', return_value=True), \
                    patch('hw_management_thermal_control.str2bool', return_value=True):

                # This would normally complete initialization, but we'll intercept
                try:
                    tc = thermal_control.ThermalManagement(self.cmd_arg)

                    # At this point, sys_config should be properly initialized
                    self.assertEqual(tc.sys_config, self.valid_sys_config)

                    # Verify that the object starts with empty sys_config that gets populated
                    self.assertIsNotNone(registered_handler)

                    # Test signal handler with properly initialized sys_config
                    with patch.object(tc, 'stop') as mock_stop:
                        try:
                            registered_handler(signal.SIGTERM)
                        except SystemExit:
                            pass  # Expected

                        # Should have called platform_support check and stop
                        mock_stop.assert_called_once()
    ...
    def test_sys_config_early_initialization_ensures_safety(self):
        """Test that sys_config is initialized to empty dict before any operations that might access it."""

        with patch('hw_management_thermal_control.Logger') as mock_logger_class, \
                patch('hw_management_thermal_control.ThermalManagement.check_file', return_value=False), \
                patch('hw_management_thermal_control.ThermalManagement.load_configuration') as mock_load_config, \
                patch('hw_management_thermal_control.ThermalManagement.is_pwm_exists', return_value=True), \
                patch('hw_management_thermal_control.str2bool', return_value=True), \
                patch('signal.signal'):
    ...
    def test_load_configuration_returns_config_instead_of_side_effect(self):
        """Test that load_configuration returns configuration instead of setting it as side effect."""
        ...
        with patch('hw_management_thermal_control.Logger') as mock_logger_class, \
            ...
            # Verify that load_configuration returns a value (not None)
            # This tests the fix where load_configuration was changed from setting
            # self.sys_config to returning the configuration
    ...
    def test_logger_close_tc_log_handler_optimization(self):
        """Test that logger close_tc_log_handler no longer calls redundant flush()."""
    ...
    def test_signal_handler_platform_support_check_with_initialized_config(self):
        """Test that signal handler properly checks platform_support from initialized sys_config."""
        ...
        for i, test_case in enumerate(test_configs):
            with self.subTest(test_case=i):
                ...
                with patch('hw_management_thermal_control.Logger') as mock_logger_class, \
                        patch('hw_management_thermal_control.ThermalManagement.check_file', return_value=False), \
                        patch('hw_management_thermal_control.ThermalManagement.load_configuration', return_value=test_config), \
                        patch('hw_management_thermal_control.ThermalManagement.is_pwm_exists', return_value=True), \
                        patch('hw_management_thermal_control.str2bool', return_value=True), \
                        patch('signal.signal'):
    ...
    def test_initialization_order_prevents_race_condition(self, mock_signal, mock_is_pwm_exists,
        ...
        """Test that initialization order prevents the race condition that was fixed."""
        ...
        # Track sys_config initialization
        original_setattr = object.__setattr__

        def track_sys_config_init(obj, name, value):
            if name == 'sys_config':
                operation_order.append(f"sys_config_set_to_{type(value).__name__}")
        ...
        with patch('hw_management_thermal_control.str2bool', return_value=True), \
                patch.object(thermal_control.ThermalManagement, '__setattr__', track_sys_config_init):

            # Create instance
            tc = thermal_control.ThermalManagement(self.cmd_arg)

            # Verify the correct initialization order:
            # 1. sys_config initialized to empty dict
            # 2. load_configuration called
            # 3. sys_config set to loaded configuration
            # 4. signal handlers registered

            expected_pattern = [
                "sys_config_set_to_dict",  # Initial empty dict
                "load_configuration_called",  # Config loading
                "sys_config_set_to_dict",  # Config assignment
            ]
...
class TestThermalInitIntegration(unittest.TestCase):
    """Integration tests for thermal control initialization fixes."""
    ...
    def test_full_initialization_flow_integration(self, mock_signal, mock_is_pwm_exists,
        ...
        """Integration test for the full initialization flow with all fixes."""
