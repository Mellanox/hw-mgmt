...
static void lpc_ivb_unconfig(struct lpci2c_priv* priv, struct pci_dev* pdev)
{
    lpc_dec_rng_unconfig(pdev, priv->lpc_gen_dec_reg[LPC_CPLD_RANGE], LPC_CPLD_RANGE);
    lpc_dec_rng_unconfig(pdev, priv->lpc_gen_dec_reg[LPC_CPLD_I2C_RANGE], LPC_CPLD_I2C_RANGE);
}

static int lpc_rng_config(struct lpci2c_priv* priv, struct pci_dev* pdev)
{
    int rc = LPCI2C_RC_OK;
    u32 val, lpc_clks;

    rc = pci_read_config_dword(pdev, LPC_RNG_LPC_CTRL, &val);
    if (rc) {
        LPCI2C_LOG_ERROR("Access to LPC Ctrl reg failed, rc %d\n", rc);
        return -EFAULT;
    }
    lpc_clks = val & 0x3;
    if (lpc_clks != LPC_CLKS_EN) {
        val &= 0xFFFFFFFC;
        rc = pci_write_config_dword(pdev, LPC_RNG_LPC_CTRL, val);
        if (rc) {
            LPCI2C_LOG_ERROR("Config of LPC CLKS CTRL failed, rc %d\n", rc);
            rc = -EFAULT;
        }
    }
	LPCI2C_LOG_DBG(2, "LPC CLKS CTRL configured\n");

    return rc;
}

static int lpc_config(struct lpci2c_priv* priv)
{
    struct pci_dev* pdev = NULL;
    u16 dev_id;
    int rc;

    pdev = pci_get_bus_and_slot(LPC_CTRL_IFC_BUS_ID, PCI_DEVFN(LPC_CTRL_IFC_SLOT_ID, LPC_CTRL_IFC_FUNC_ID));
    if (!pdev) {
        LPCI2C_LOG_ERROR("LPC controler bus:%d slot:%d func:%d doesn't found\n",\
                       LPC_CTRL_IFC_BUS_ID, LPC_CTRL_IFC_SLOT_ID, LPC_CTRL_IFC_FUNC_ID);
        return -EFAULT;
    }

    rc = pci_read_config_word(pdev, 2, &dev_id);
    if (rc) {
        LPCI2C_LOG_ERROR("Access PCIe LPC Ifc failed, rc %d\n", rc);
        goto failure;
    }

    switch (dev_id) {
    case LPC_QM67_DEV_ID:
    case LPC_QM77_DEV_ID:
        rc = lpc_ivb_config(priv, pdev);
        break;
    case LPC_RNG_DEV_ID:
        rc = lpc_rng_config(priv, pdev);
        break;
    default:
        rc = -ENXIO;
        LPCI2C_LOG_ERROR("Unsupported DevId 0x%x on bus:%d slot:%d func:%d\n",
                         dev_id, LPC_CTRL_IFC_BUS_ID, LPC_CTRL_IFC_SLOT_ID, LPC_CTRL_IFC_FUNC_ID);
        goto failure;
    }
    priv->dev_id = dev_id;
	LPCI2C_LOG_DBG(2, "LPC controler 0x%x bus:%d slot:%d func:%d configured\n",\
                     priv->dev_id, LPC_CTRL_IFC_BUS_ID, LPC_CTRL_IFC_SLOT_ID, LPC_CTRL_IFC_FUNC_ID);
...
static int lpc_unconfig(struct lpci2c_priv* priv)
{
    struct pci_dev* pdev = NULL;
    int rc = LPCI2C_RC_OK;
	...
	LPCI2C_LOG_DBG(2, "LPC controler 0x%x bus:%d slot:%d func:%d unconfigured\n",\
                     priv->dev_id, LPC_CTRL_IFC_BUS_ID, LPC_CTRL_IFC_SLOT_ID, LPC_CTRL_IFC_FUNC_ID);
...
static int __init lpci2c_init(void)
{
    ...
    rc = lpc_config(priv);
    if (rc) {
        LPCI2C_LOG_ERROR("Failed to configure LPC interface\n");
	...
	rc = lpc_region_request(priv);
    if (rc) {
        LPCI2C_LOG_ERROR("Request ioregion failed (%d)\n", rc);
        goto fail_platform_device1;
    }
    ...
    if (force_irq) {
        ...
        if (rc) {
            ...
            LPCI2C_LOG_DBG(2, "Irq %d registered\n", LPCI2C_IRQ_NUM);
        }
    }
    ...
    adap->timeout = usecs_to_jiffies(xfer_to);

    priv->poll_time = poll_time;
	...
	if (rc) {
        ...
        LPCI2C_LOG_DBG(2, "Add %s adapter to bus %d is done\n", LPCI2C_DEVICE_NAME, LPCI2C_BUS_NUM);
	...
	printk(KERN_INFO "%s  Version %s\n", LPCI2C_DRV_DESCRIPTION, LPCI2C_DRV_VERSION);
...
MODULE_DESCRIPTION(LPCI2C_DRV_DESCRIPTION);
MODULE_VERSION(LPCI2C_DRV_VERSION);
...
MODULE_PARM_DESC(xfer_to, " transfer timeout in microsec, default - 500000");
MODULE_PARM_DESC(poll_time, " polling time in microsec, default - 2000");
