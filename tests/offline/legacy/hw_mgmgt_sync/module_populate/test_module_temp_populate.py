...
class TestModuleTempPopulate(unittest.TestCase):
    ...
    def _load_hw_management_module(self, hw_mgmt_path):
        ...
        # Mock the LOGGER global variable that's used in module_temp_populate
        if not hasattr(hw_mgmt_module, 'LOGGER') or hw_mgmt_module.LOGGER is None:
            hw_mgmt_module.LOGGER = MagicMock()
            hw_mgmt_module.LOGGER.debug = MagicMock()
            hw_mgmt_module.LOGGER.info = MagicMock()
            hw_mgmt_module.LOGGER.warning = MagicMock()
            hw_mgmt_module.LOGGER.error = MagicMock()
        ...
        with patch('builtins.open', side_effect=mock_open_func), \
            ...
            for idx, config in enumerate(self.module_configs):
                ...
                if config['mode'] == 1:  # SDK_SW_CONTROL
                    ...
                    print(f"[PASS] Module {module_name}: SW control mode - no files created")

                else:  # SDK_FW_CONTROL
                    if config['present'] == 0:
                        # Files should contain zeros for absent modules
                        self._verify_absent_module_files(module_name)
                        print(f"[PASS] Module {module_name}: FW control, not present - zero values")
                        ...
                        print(f"[PASS] Module {module_name}: FW control, present - actual values "
