...
class TestChipupStatusFiles(unittest.TestCase):
    ...
    def _load_thermal_module(self):
        """Load thermal_updater module dynamically"""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        repo_root = os.path.join(script_dir, '..', '..', '..')
        hw_mgmt_dir = os.path.join(repo_root, 'usr', 'usr', 'bin')
        thermal_path = os.path.join(hw_mgmt_dir, 'hw_management_thermal_updater.py')
    ...
    def test_01_single_asic_chipup_completed(self):
        ...
        thermal_module = self._load_thermal_module()

        # Mock LOGGER
        mock_logger = MagicMock()
        thermal_module.LOGGER = mock_logger
                ...
                with patch('os.path.exists', return_value=True):
                    with patch('os.path.islink', return_value=False):
                        with patch('os.symlink'):
                            ...
                            thermal_module.asic_temp_populate(arg_list, None)
    ...
    def test_06_integration_chipup_files_always_updated(self):
        ...
        # Verify asic_temp_populate function exists and writes chipup files
        self.assertTrue(hasattr(thermal_module, 'asic_temp_populate'))
        self.assertTrue(callable(thermal_module.asic_temp_populate))

        # Check function docstring mentions chipup
        docstring = thermal_module.asic_temp_populate.__doc__
        self.assertIsNotNone(docstring)

        print("[PASS] asic_temp_populate correctly updates chipup status files")
