...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_run_power_button_event(self):
        ...
        with patch('hw_management_peripheral_updater.os.system') as mock_system:
            ...
            # logger + NVMe ready first, then 2 hotplug events
            self.assertEqual(mock_system.call_count, 4)
            calls = [str(c) for c in mock_system.call_args_list]
            self.assertTrue("hw-management-bmc-nvme-ready.sh" in calls[1])
            chassis = [c for c in calls if "hw-management-chassis-events.sh" in c]
            self.assertEqual(len(chassis), 2)
            self.assertTrue(all("HW_MGMT_NVME_READY=0" in c for c in chassis))
