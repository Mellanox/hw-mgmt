...
def test_systemd_restart_brings_back_tc_that_was_running():
    """systemd stops TC before this script runs, so the saved state is what restores it."""
    s = _run_bash_cmdline({"STATE": "started", "TC_IS_EN": "1", "TC_IS_ACT": "0"})
    assert "start hw-management-tc" in s


def test_systemd_restart_leaves_manually_stopped_tc_down():
    """No saved state means TC was not running: it must not be started or stopped."""
...
def test_direct_script_run_restarts_tc_onto_recreated_tree():
    """hw-management.sh run by hand: nothing stopped TC, but its sysfs tree is new."""
    s = _run_bash_cmdline({"STATE": "started", "TC_IS_EN": "1", "TC_IS_ACT": "1"})
    assert "restart hw-management-tc" in s
...
def test_tc_state_is_saved_on_hw_management_stop_and_restored_on_start():
    ...
    assert "save_tc_state_started()" in helpers
    assert "consume_tc_saved_state()" in helpers
    # Helpers: path near other /run state files; functions after check_tc_is_supported.
    assert helpers.index("asic_chipup_status=") < helpers.index('tc_state_file="/var/run/.hw-management-tc-state"')
    assert helpers.index("check_tc_is_supported()") < helpers.index("save_tc_state_started()")
    ...
    # hw-management.sh stop saves at the end of do_stop.
    do_stop = hw_management[hw_management.index("do_stop()"): hw_management.index("function find_asic_hwmon_path")]
    assert "save_tc_state_started" in do_stop
    assert do_stop.index("rm -fR /var/run/hw-management") < do_stop.index("save_tc_state_started")
...
def test_save_tc_state_started_is_mode_600_and_overwrites_symlink(tmp_path):
    root = Path(__file__).resolve().parents[2]
    helpers = root / "usr" / "usr" / "bin" / "hw-management-helpers.sh"
    state = tmp_path / ".hw-management-tc-state"
    target = tmp_path / "elsewhere"
