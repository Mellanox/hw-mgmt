...
(+x), free space, dump lock (non-blocking), and `--outdir` rules
(protected / symlink in any component / non-empty custom / not
writable) without creating or deleting that directory. It does
**not** run the vendor tool or write dump files. Prints status
fields to stdout (rc 0 = ok or skipped, 1 = warning).
...
`--quiet` hides stderr. With `--verify` it also hides the
status fields on stdout (rc still 0/1). Syslog remains. No NVMe
...
already collecting, or if the lock file cannot be opened). On that
skip it still copies last good `$SSD_TAR` into the hw-mgmt dump.
Standalone Python waits up to 200 s.
