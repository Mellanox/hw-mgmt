...
following **AN001-XDPE1x2xx Programming Guide** behavior (section references live in
source comments, not in every log line).
...
With **no** arguments, usage is printed (same as `-h`).
...
- Infineon **AN001** programming guide (PDF in vendor collateral).
