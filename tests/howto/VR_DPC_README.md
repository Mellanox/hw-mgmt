...
Userland tools below support **MPS** (`mp*`), **Infineon** (`xdpe*`), and
**Renesas** (`raa*` / `rrv*`) VR DPC updates. Infineon skip-identical /
`--verify` targets come from GUI `[User Data]` (`USER_DATA_01` / `USER_DATA_00`).
...
| `hw-management-vr-dpc-update-all.sh` | Batch update from a **JSON file** (MPS + Infineon + Renesas, vendor auto-detect) |
...
| `hw-management-vr-dpc-renesas-update.sh` | Flash one **Renesas** device (Infineon-like CLI: `flash -y -b -a -f`) |
...
### DPC package (tar.gz) — preferred for mixed vendor packages
...
Infineon/Renesas: `Addr`, no MPS-only fields). JSON `Bus` is the **absolute** I2C bus
(no `i2c_bus_offset` applied by the updaters). Infineon entries are flashed via
`hw-management-vr-dpc-infineon-update.sh flash -y`; Renesas via
`hw-management-vr-dpc-renesas-update.sh flash -y`.
...
- **2026-08:** Clarify absolute JSON `Bus`; link both example JSON files.
